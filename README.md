...
- 电商平台项目从11.19 开始一直到12.7 ,用户模块，商品模块，购物车模块写完。订单模块还没有写，准备1月25之前搞定，最起码上线，不能再拖了！！！！

- 12.9号开始整理前端面试项目，新增渲染机制，修改布局，原型，原型链，new，instance of ，面向对象，继承，ajax，一直到12.22
- 12.22 之后就准备开题报告，去武汉，
- 2019.1.2，复习了面向对象，开始学习数据结构和算法知识，用数组实现栈，队列，链表，BST
...
基础内容 基础语法 MVVM模式 组件化 生命周期 动画特效 实战项目 环境搭建 使用Git 数据模拟 本地开发 联调 真机测试 上线 Axios Vue Router Vuex 异步组件 Stylus 插件 递归组件 公共组件

2019.03.23 更新

MVP 模式 数据 视图 逻辑（控制器） MVVM 模式 数据的双向绑定 前端组件化， 组件是页面的一部分，一个区域 组件间传值 父向子 通过v-bind 传递，同时子组件通过props 接收 子组件向父组件传值 @emit 触发父组件定义事件

v-bind 响应式更新HTML 简写 ： v-on 监听dom 事件 简写 @ 2019.03.26 更新

vue 的生命周期钩子 每个 Vue 实例在被创建时都要经过一系列的初始化过程——例如，需要设置数据监听、编译模板、将实例挂载到 DOM 并在数据变化时更新 DOM 等

beforeCreate 注入，绑定 created

beforeMount渲染页面 mounted

beforeUpdate 绑定数据 updated
...
条件渲染,v-if 如果是false 不存在于dom，v-show 只是display none

列表渲染,数组和对象更新检测的几种方法 2019.4.11 更新

深入理解vue组件 1.is 的使用解决h5 标签的问题 2. 子组件定义data为函数 3. ref的使用，获取dom 2019.4.17 更新

放弃学习前端 转IC 如果没有遇见你，我将会是在哪里，日子过得怎么样？ 人生是否要珍惜，不知道会不会，也有爱情甜如蜜。任时光匆匆六区
